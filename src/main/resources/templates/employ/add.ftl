...
<form class="layui-form layui-form-pane" action="" id="form" >
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <select name="pname" id="sta" lay-verify="required"  lay-search="">

            </select>
        </div>
        <label class="layui-form-label">发布日期<span style="color: #FF5722">*</span></label></label>
        <div class="layui-input-block">
            <input type="text" name="time" autocomplete="off" class="layui-input"
                   lay-verify="required"    id="test2" placeholder="yyyy-MM-dd"   >
        </div>
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(var i=0;i<data.length;i++){
                    options+= '<option value="'+data[i].name+'|'+data[i].id+'">'+data[i].name+'</option>';
                }
                $("#sta").html(options);
                form.render();
            }
